...
class LowLevelPolicy:
    ...
    def __init__(self, args):
        ...
        self.server_endpoint = f"http://{args.server_url}:{args.port}/act"
        print(f"Using server endpoint: {self.server_endpoint}")
    ...
    def main(self) -> None:
        ...
        while t < self.num_inferences:
            try:
                ...
                observation = self.get_observation_dvrk()
        
                # Get prediction from server
                action = self.get_action_from_server(observation)
                if action is None:
                    print(f"Warning: Failed to get prediction for sample {t}, skipping")
                    continue
